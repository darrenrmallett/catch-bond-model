...
def optimal_to_unloaded_lifetime( forces_and_lifetimes ):
    
    # calculates the ratio between the highest point (the lifetime at the optimal force) 
    # and the starting lifetime (lifetime at unloaded force)
    
    list_of_lifetimes = list(forces_and_lifetimes.values())
    list_of_forces = list(forces_and_lifetimes.keys())
    
    optimal_lifetime = forces_and_lifetimes[optimal_force( forces_and_lifetimes )]
    starting_lifetime = list_of_lifetimes[0]
    
    ratio = optimal_lifetime/starting_lifetime
...
for k4_UR in k4_list : 
    ...
    for k3_UR in k3_list:
        ...
        ratio = optimal_to_unloaded_lifetime(forces_and_lifetimes)
        ...
        k3_k4_values[k4_UR_str][k3_UR_str] = ratio
...
output += f'# This is using the MILLER WILD-TYPE DATA (EXCEPT k3 and k4 sensitivites are extrapolated from Franck 2007 datapoints curve-fit to the best of my abilities with exponential functions!\n'
output += f'# Output: Optimum ratio = optimum lifetime/starting lifetime\n\n'
...
filehandle = open('optimum_ratio_MillerWT_Franck2007_Dam1k3_k4_sensitivities_vary_k3UR_and_k4UR.txt','w')
